...
public class MessageRepository {
	...
	private ChatMessage insert(ChatMessage msg) {
		...
		String sql = "INSERT INTO chat_messages (room_id, author_id, message_type, content"+
				(msg.getParent_msg_id()!=null?", parent_message_id":"")+") VALUES ( ?, ?, ?, ?"+
				(msg.getParent_msg_id()!=null?", ?":"")+" )";
		List<String> Params = new ArrayList<>(Arrays.asList(""+msg.getRoom_id(), ""+msg.getAuthor_id(), msg.getMsg_type().name(), msg.getContent()));
	...
	private ChatMessage mapRowToMsg(Map<String, Object> row) {
		ChatMessage msg = new ChatMessage((Long) row.get("message_id"), (Long) row.get("author_id"), (Long) row.get("room_id"));
