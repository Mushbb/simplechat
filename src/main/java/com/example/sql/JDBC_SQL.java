...
public class JDBC_SQL {
    ...
    public static Map<String, Object> executeUpdate(String sqlQuery, String[] Params, String[] returnCols) {
        ...
        try {
            ...
            // 3. Statement 객체 생성 (SQL 쿼리 실행용) + 파라미터 등록
            statement = connection.prepareStatement(sqlQuery, returnCols);
            if( Params != null )
	            for(int i=0;i<Params.length;i++)
	            	statement.setObject(i+1, Params[i]);
